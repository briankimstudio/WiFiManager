...
void WiFiManager::setupConfigPortal() {
  ...
  server.reset(new WifiManagerWebServerType(80));
  ...
  server->on("/wifi", std::bind(&WiFiManager::handleWifi, this, std::placeholders::_1, true));
  server->on("/0wifi", std::bind(&WiFiManager::handleWifi, this, std::placeholders::_1, false));
  server->on("/wifisave", std::bind(&WiFiManager::handleWifiSave, this, std::placeholders::_1));
  server->on("/i", std::bind(&WiFiManager::handleInfo, this, std::placeholders::_1));
  server->on("/r", std::bind(&WiFiManager::handleReset, this, std::placeholders::_1));
...
boolean  WiFiManager::startConfigPortal(char const *apName, char const *apPassword) {
  ...
  // brian
  // WiFi.scanNetworks won't work within espasyncwebserverhandler, so, 
  // it calls async version in advance and let handler use the results later.
  WiFi.scanNetworksAsync([](int ret){});
  ...
  //brian
  // After WiFiManager, our own webserver can not bind the same web port occasionally.
  // After reboot, it skips WiFiManager's webserver and our own webserver works fine.
  // This is not nessary if you don't start your own web server.
  ESP.restart();
...
void WiFiManager::handleRoot(WifiManagerWebServerRequestType *request) {
...
void WiFiManager::handleWifi(WifiManagerWebServerRequestType *request, boolean scan) {
  String page = FPSTR(HTTP_HEADER);
...
void WiFiManager::handleWifiSave(WifiManagerWebServerRequestType *request) {
...
void WiFiManager::handleInfo(WifiManagerWebServerRequestType *request) {
  DEBUG_WM(F("Info"));
...
void WiFiManager::handleReset(WifiManagerWebServerRequestType *request) {
  DEBUG_WM(F("Reset"));
...
void WiFiManager::handleNotFound(WifiManagerWebServerRequestType *request) {
...
boolean WiFiManager::captivePortal(WifiManagerWebServerRequestType *request) {
